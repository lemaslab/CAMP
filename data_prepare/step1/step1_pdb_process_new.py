...
def load(pdb_pep_dataset, plip_result_filename):  # pdb_pep_chain   #plip_predict_result
    ...
    for i in range(df_fasta_pep.shape[0]):
        ...
        result_file_name = './peptide_result/' + pdb_id + '_' + chain.upper() + '_result.txt'
...
# OUTPUT: -
def load_all_fasta(all_fasta_file, input_dataset):  # pdbid_all_fasta_demo # df_predict (plip_predict_result)
    # import data
    ...
    df_predict_det = pd.merge(input_dataset, df_fasta_vocabulary, how='left', left_on=['pdb_id', 'pep_chain'],
...
def map_uniprot_chain(input_dataset, pdb_chain_uniprot):  # df_predict_det1 #pdb_chain_uniprot.tsv
    ...
    file_name = pdb_chain_uniprot
    df_uni_id.to_csv(file_name, encoding='utf-8', index=False, sep='\t')
    df_predict_det3.to_csv("df_predict_det3.tsv", encoding='utf-8', index=False, sep='\t')
    return df_predict_det2_no_uni, df_predict_det3


df_predict = load('pdb_pep_chain', 'plip_predict_result')
df_predict_det1 = load_all_fasta('subset_pdbid_all_fasta', df_predict)
df_predict_det2_no_uni, df_predict_det3 = map_uniprot_chain(df_predict_det1, 'pdb_chain_uniprotnew.tsv')



def load_uni_seq(input_dataset,uniprot2seq_file, df_predict_det2_no_uni):
    ...
    # df_uni2seq.to_csv('df_uni2seq.tsv', index=False, encoding='utf-8', sep='\t')
    # df_predict_det4.to_csv('df_predict_det4.tsv', index=False, encoding='utf-8', sep='\t')
    # exit(0)
    # filter out MHC
...
df_pdb_pairs = load_uni_seq(df_predict_det3,'uniprot2seq2.txt', df_predict_det2_no_uni)
